...
export function DashboardPage() {
  const { data: projects = [], isLoading: isLoadingProjects } = useQuery<IProject[]>({
    queryKey: ["projects"],
    queryFn: async () => {
  ...
  const stats = [
    ...
    { title: "Total Tasks", value: totalTasks.toString(), desc: `${pendingTasks} pending`, icon: CheckSquare, color: "text-orange-500" },
  ...
  const projectChartData = projects.map((p) => ({ name: p.title, progress: p.progress }));

  const managers = users.filter((u) => u.role === "Admin" || u.role === "Developer");
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-7">
        ...
        <div className="rounded-xl border bg-card text-card-foreground shadow-sm lg:col-span-3 p-4">
          ...
          <div className="h-[300px] mt-4">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={projectChartData} margin={{ top: 10, right: 30, bottom: 0 }} layout="vertical">
                <CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="hsl(var(--border))" />
                <XAxis type="number" stroke="hsl(var(--muted-foreground))" fontSize={12} tickLine={false} axisLine={false} domain={[0, 100]} />
                <YAxis dataKey="name" type="category" stroke="hsl(var(--muted-foreground))" fontSize={11} tickLine={false} axisLine={false} width={150} interval={0} />
                <Tooltip
                  contentStyle={{ backgroundColor: "hsl(var(--popover))", borderColor: "hsl(var(--border))", borderRadius: "8px" }}
                  itemStyle={{ color: "hsl(var(--foreground))" }}
            ...
            <div className="space-y-3">
              {projects.map((project) => (
                ...
                >
                  ...
                  <Badge
                    variant={project.status === "in_progress" ? "secondary" : "outline"}
              ...
              {tasks.slice(0, 6).map((task) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className={`h-2 w-2 rounded-full ${statusColorMap[task.status]}`} />
