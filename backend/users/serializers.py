...
class UserRegisterSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['id', 'name', 'username', 'email', 'first_name', 'last_name',
                  'password', 'role', 'phone']
...
class UserSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        extra_kwargs = {
            # ✅ username is read-only on update — handled by perform_update in the view
...
class PasswordResetRequestSerializer(serializers.Serializer):
    ...
    def save(self, **kwargs):
        ...
        if not settings.EMAIL_HOST or not settings.EMAIL_HOST_USER or not settings.EMAIL_HOST_PASSWORD:
            raise serializers.ValidationError({
                "email": "Email service is not configured."
            })
    ...
    def validate(self, attrs):
        ...
        token = (
            PasswordResetToken.objects.filter(
                user=user, used_at__isnull=True, verified_at__isnull=False
            )
            .order_by("-created_at")
            .first()
        )
